 ...
 * @LastEditors: szhao
 * @LastEditTime: 2023-01-18 10:22:06
 * @Description: 视频播放器
-->
<template>
    <div
    ...
    >
        ...
        >
            <IconFont
                ...
                :type="
                    isFlullscreen ? 'icon-tuichuquanping' : 'icon-a-9Equanping'
                "
...
export default {
    ...
    setup(prop, ctx) {
        ...
        function flvInitLoad() {
            ...
            flvDestory();
            addLog(
                `flv init load,loading:${state.loading},address:${prop.url}`
            );
        ...
        function videoElementEvent() {
            ...
            videoElement.onprogress = (e) => {
                ...
                if (
                    !prop.isBufferedEnd ||
                    state.flvPlayer.buffered.length <= 0
                ) {
                    return;
                }
                state.loading = false;
                ...
                if (diff > 20 || diff < 0) {
                    ...
                    state.flvPlayer.currentTime =
                        state.flvPlayer.buffered.end(0) - 0.5; // 手动跳帧到最后
                    return;
                }
                // 正常帧率，正常播放
                if (diff <= 1) {
        ...
        function errorHandle() {
            ...
            if (
                state.flvPlayer &&
                state.errorCount <= state.maxReconnectCount
            ) {
        ...
        function statisticsInfoHanle(res) {
            ...
            // 播放异常
                ...
                if (state.player) {
                    addLog(
                        `Reconnect after video freezes, address:${prop.url}`
                    ); // 添加日志
                    state.errorCount = 0; // 错误连接次数归0
